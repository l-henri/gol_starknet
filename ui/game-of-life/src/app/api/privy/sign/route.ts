import { AuthError, jsonError, privy, privyConfigured, requireUserId, starknetWalletsOf } from "@/lib/privyServer";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const res = await privy().wallets().rawSign(walletId, { params: { hash: hash as `0x${string}` } });
